...
# =========================================================
# CADASTRO DE GRANDE ÁREA / TEMA / SUBTÓPICO
# =========================================================

def init_taxonomia_tables():
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()

    cur.execute("""
        CREATE TABLE IF NOT EXISTS grande_areas (
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS temas (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            grande_area_id INTEGER NOT NULL,
            nome TEXT NOT NULL,
            UNIQUE(grande_area_id, nome),
            FOREIGN KEY (grande_area_id) REFERENCES grande_areas(id)
        )
    """)
...
def listar_grandes_areas():
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT id, nome FROM grande_areas ORDER BY nome ASC")
    rows = cur.fetchall()
    conn.close()
    return rows


def listar_temas(grande_area_id):
    if not grande_area_id:
        return []
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
...
def obter_ou_criar_grande_area(nome: str):
    ...
    cur.execute("SELECT id FROM grande_areas WHERE LOWER(nome)=LOWER(?)", (nome,))
    row = cur.fetchone()
    ...
    cur.execute("INSERT INTO grande_areas (nome) VALUES (?)", (nome,))
    conn.commit()
...
def obter_ou_criar_tema(grande_area_id, nome: str):
    nome = normalizar_texto(nome)
    if not grande_area_id or not nome:
        return None
    ...
    cur.execute("""
        SELECT id
        FROM temas
        WHERE grande_area_id = ? AND LOWER(nome)=LOWER(?)
    """, (grande_area_id, nome))
    row = cur.fetchone()
    ...
    cur.execute("""
        INSERT INTO temas (grande_area_id, nome)
        VALUES (?, ?)
    """, (grande_area_id, nome))
    conn.commit()
    novo_id = cur.lastrowid
...
def obter_ou_criar_subtopico(tema_id, nome: str):
    nome = normalizar_texto(nome)
    if not tema_id or not nome:
        return None
    ...
    cur.execute("""
        SELECT id
        FROM subtopicos
        WHERE tema_id = ? AND LOWER(nome)=LOWER(?)
    """, (tema_id, nome))
    row = cur.fetchone()
    ...
    cur.execute("""
        INSERT INTO subtopicos (tema_id, nome)
        VALUES (?, ?)
    """, (tema_id, nome))
    conn.commit()
    novo_id = cur.lastrowid
...
def semear_taxonomia_padrao():
    exemplos = {
        "Clínica Médica": {
            "Cardiologia": ["Arritmias", "Insuficiência Cardíaca", "SCA", "Valvopatias"],
            "Pneumologia": ["Asma", "DPOC", "Tromboembolismo Pulmonar"],
        },
        "Cirurgia": {
            "Abdome Agudo": ["Apendicite", "Colecistite", "Obstrução Intestinal"],
        },
        "Pediatria": {
            "Neonatologia": ["Icterícia Neonatal", "Sepse Neonatal"],
        },
        "Ginecologia e Obstetrícia": {
    ...
    for ga_nome, temas in exemplos.items():
        ga_id = obter_ou_criar_grande_area(ga_nome)
        for tema_nome, subs in temas.items():
            tema_id = obter_ou_criar_tema(ga_id, tema_nome)
            for sub_nome in subs:
                obter_ou_criar_subtopico(tema_id, sub_nome)
...
def draw_pdf_header(c, subtitle="Relatório Estratégico"):
    ...
    c.drawString(3.1 * cm, height - 0.95 * cm, "Nexo Med")
...
def draw_pdf_footer(c, username=""):
    footer_text = "Nexo Med • Diagnóstico situacional premium"
...
def init_db():
    ...
    # CATÁLOGO DE GRANDE ÁREA -> TEMA -> SUBTÓPICO
    cur.execute("""
...
def ensure_schema_upgrades():
    ...
    try:
        ...
        study_cols = [row[1] for row in cur.fetchall()]
    except Exception:
        study_cols = []

    if "grande_area" not in study_cols:
...
def get_target_user_options(include_current_user=True):
    ...
    if include_current_user and st.session_state.get("user_id") and not st.session_state.get("is_admin", False):
    ...
    if not options and include_current_user and st.session_state.get("user_id"):
        options.append({
            "label": st.session_state.get("username", "Usuário atual"),
...
def resolve_selected_target_user_ids(selected_labels):
    ...
    user_ids = [label_map[x] for x in selected_labels if x in label_map]
    user_ids = sorted(list(set(user_ids)))
    return user_ids


# =========================================================
# CATÁLOGO DE TEMAS E SUBTÓPICOS
# =========================================================
...
def add_catalog_item(user_id: int, grande_area: str, subject: str, topic: str):
    grande_area = normalize_text(grande_area)
    subject = normalize_text(subject)
    topic = normalize_text(topic)

    if not grande_area:
        return False, "Selecione a grande área."
    if not subject:
        return False, "Digite o tema."
    if not topic:
        return False, "Digite o subtópico."
    ...
    try:
        ...
        if row:
            return False, "Esse tema/subtópico já está cadastrado."

        cur.execute("""
            INSERT INTO subject_topic_catalog (
                user_id, grande_area, subject, topic, created_at
            )
            VALUES (?, ?, ?, ?, ?)
        """, (
            user_id,
        ...
        conn.commit()
        return True, "Tema e subtópico cadastrados com sucesso."
    except Exception as e:
        return False, f"Erro ao cadastrar tema/subtópico: {e}"
    finally:
        conn.close()


def add_catalog_subject_only(user_id: int, grande_area: str, subject: str):
    grande_area = normalize_text(grande_area)
    subject = normalize_text(subject)
    ...
    try:
        cur.execute("""
            ...
            WHERE user_id = ?
              ...
              AND topic = ''
            LIMIT 1
        """, (user_id, grande_area, subject))
        row = cur.fetchone()

        if row:
            return False, "Esse tema já está cadastrado."
        ...
        conn.commit()
        return True, "Tema cadastrado com sucesso."
    except Exception as e:
        return False, f"Erro ao cadastrar tema: {e}"
    finally:
        conn.close()


def add_catalog_topic_to_existing_subject(user_id: int, grande_area: str, subject: str, topic: str):
    return add_catalog_item(user_id, grande_area, subject, topic)


def fetch_catalog_df(user_id: int):
    df = fetch_dataframe("""
        SELECT *
        FROM subject_topic_catalog
        WHERE user_id = ?
        ORDER BY grande_area ASC, subject ASC, topic ASC, id ASC
    """, (user_id,))

    if df.empty:
        return pd.DataFrame(columns=["id", "user_id", "grande_area", "subject", "topic", "created_at"])

    for col in ["grande_area", "subject", "topic", "created_at"]:
        df[col] = df[col].fillna("").astype(str)

    return df


def fetch_subjects_by_area(user_id: int, grande_area: str):
    grande_area = normalize_text(grande_area)
    if not grande_area:
        return []
    ...
    if df.empty:
        return []
    return [normalize_text(x) for x in df["subject"].tolist() if normalize_text(x)]


def fetch_topics_by_area_subject(user_id: int, grande_area: str, subject: str):
    grande_area = normalize_text(grande_area)
    subject = normalize_text(subject)
    if not grande_area or not subject:
        return []
    ...
    if df.empty:
        return []
    return [normalize_text(x) for x in df["topic"].tolist() if normalize_text(x)]


def delete_catalog_item(catalog_id: int):
    conn = get_conn()
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM subject_topic_catalog WHERE id = ?", (catalog_id,))
        conn.commit()
        return True
    except Exception:
        return False
...
def add_subject_topic_catalog_if_missing(user_id: int, grande_area: str, subject: str, topic: str):
    grande_area = normalize_text(grande_area)
    subject = normalize_text(subject)
    topic = normalize_text(topic)
...
def bootstrap_catalog_from_existing_data(user_id: int):
    # sessões
    sessions_df = fetch_dataframe("""
        SELECT grande_area, subject, topic
        FROM study_sessions
        WHERE user_id = ?
    """, (user_id,))
    ...
    if not sessions_df.empty:
        for _, row in sessions_df.iterrows():
            add_subject_topic_catalog_if_missing(
                user_id=user_id,
    ...
    if not schedule_df.empty:
        for _, row in schedule_df.iterrows():
            add_subject_topic_catalog_if_missing(
                user_id=user_id,
                grande_area=normalize_text(row.get("area", "")),
                subject=normalize_text(row.get("subject", "")),
                topic=normalize_text(row.get("topic", "")),
            )

    # flashcards
    flash_df = fetch_dataframe("""
...
def get_subject_options_for_area(user_id: int, grande_area: str):
    subjects = fetch_subjects_by_area(user_id, grande_area)
    return subjects if subjects else []


def get_topic_options_for_area_subject(user_id: int, grande_area: str, subject: str):
    topics = fetch_topics_by_area_subject(user_id, grande_area, subject)
...
def inject_global_css():
    st.markdown(
        ...
        .hero-dynamic-title{
            ...
            animation: typingTitle 6s steps(20,end) infinite, blinkCaret .9s step-end infinite;
        }
...
def render_auth_hero():
    ...
    st.markdown(
        ...
        <div class="hero-title">
            ...
            <span class="hero-dynamic-title">Nexo Med</span>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        <div class="hero-subtext">
            ...
            em uma experiência visual elegante, sólida e profissional.
        </div>
        """,
        unsafe_allow_html=True
    )
...
def render_login_screen():
    ...
    with col_right:
        ...
        st.markdown(
            '<div class="login-subtitle">Acesse seu painel e continue sua execução estratégica de estudos com uma experiência premium, limpa e profissional.</div>',
            unsafe_allow_html=True
        )
        ...
        with tab_login:
            ...
            if submitted:
                ...
                if user:
                    ...
                    bootstrap_catalog_from_existing_data(user["id"])
        ...
        st.markdown(
            """
            <div class="mini-note" style="margin-top:16px;">
                Dica: use seu painel diariamente para consolidar questões, revisão e cronograma no mesmo fluxo.
            </div>
            """,
            unsafe_allow_html=True
        )
...
def build_dashboard_metrics(user_id: int):
    ...
    ranking_df = pd.DataFrame(columns=["topic_display", "accuracy", "questions_done", "correct_answers", "grande_area", "subject"])
    if not sessions_df.empty:
    ...
    review_df = build_questions_review_df(sessions_df)
    upcoming_reviews_df = pd.DataFrame(columns=review_df.columns if not review_df.empty else [
        "grande_area", "subject", "topic", "topic_display", "questions_done",
        "correct_answers", "study_minutes", "accuracy", "last_session_date",
        "review_days", "review_label", "priority_label", "priority_order",
        "next_review_date", "review_status"
    ])

    if not review_df.empty:
        base_reviews = review_df.copy()
        today_iso = date.today().isoformat()
        soon_limit = (date.today() + timedelta(days=7)).isoformat()

        upcoming_reviews_df = base_reviews[
            (base_reviews["next_review_date"] >= today_iso) &
            (base_reviews["next_review_date"] <= soon_limit)
        ].copy()

        upcoming_reviews_df = upcoming_reviews_df.sort_values(
            ["next_review_date", "priority_order", "accuracy"],
...
def render_priorities_panel(metrics: dict):
    ...
    st.markdown('<div class="exec-sub">Leitura rápida das metas operacionais.</div>', unsafe_allow_html=True)
...
def render_ranking_10_panel(metrics: dict):
    ...
    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown('<div class="exec-title">10 melhores e 10 piores subtópicos</div>', unsafe_allow_html=True)
    st.markdown('<div class="exec-sub">Com semáforo de desempenho: vermelho &lt; 70 • amarelo 70 a 80 • verde com troféu &gt; 80.</div>', unsafe_allow_html=True)
...
def render_upcoming_reviews_panel(metrics: dict):
    reviews_df = metrics["upcoming_reviews_df"].copy()

    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown('<div class="exec-title">Revisões perto de vencer</div>', unsafe_allow_html=True)
    st.markdown('<div class="exec-sub">Assuntos que vencem nos próximos 7 dias para antecipar sua revisão.</div>', unsafe_allow_html=True)

    if reviews_df.empty:
        st.markdown('<div class="b3-empty">Nenhuma revisão próxima do vencimento nos próximos 7 dias.</div>', unsafe_allow_html=True)
    else:
        for _, row in reviews_df.iterrows():
            ...
            meta_parts.append(f"Revisão: {next_review_date}")
            ...
            st.markdown(
                (
                    ...
                    f'<div class="overview-schedule-meta">{html.escape(" • ".join(meta_parts))}</div>'
                    '</div>'
                    f'<div>{chip_html}</div>'
                    '</div>'
                    f'<div class="overview-schedule-meta" style="margin-top:6px;">Rendimento: </div>'
                    f'<div style="margin-top:4px;">{get_performance_badge_html(acc)}</div>'
                    '</div>'
                ),
                unsafe_allow_html=True
            )
...
def render_strategy_panel(metrics: dict):
    ...
    st.markdown('<div class="exec-sub">Panorama central da rotina de estudo e metas atuais.</div>', unsafe_allow_html=True)
...
def render_goal_settings_panel(user_id: int, title: str = "Minhas metas", subtitle: str = "Todos os usuários podem ajustar suas metas pelo estágio."):
    ...
    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown(f'<div class="exec-title">{html.escape(title)}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="exec-sub">{html.escape(subtitle)}</div>', unsafe_allow_html=True)
...
def render_visao_geral():
    ...
    st.markdown('<div class="section-subtitle">Seu cockpit premium com foco em rotina, desempenho, metas, cronograma e revisões próximas.</div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
def add_schedule_item_for_users(target_user_ids, week_no: int, area: str, subject: str, topic: str,
...
def import_schedule_from_csv_for_users(target_user_ids):
    ...
    try:
        ...
        conn.commit()
        return True, f"Cronograma importado com sucesso para {len(target_user_ids)} usuário(s)."
    except Exception as e:
...
def render_schedule_manager():
    ...
    st.markdown('<div class="section-subtitle">Cadastre, acompanhe e finalize tarefas com envio para usuário específico, multiusuários ou todos.</div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
def get_questions_taxonomy_from_csv():
    ...
    try:
        ...
        for _, row in df.iterrows():
            grande_area = normalizar_texto(row.get(area_col, ""))
            nome_tarefa = normalizar_texto(row.get(nome_col, ""))
            ...
            if " - " in nome_tarefa:
                ...
                tema = normalizar_texto(nome_tarefa)
                subtopico = "Geral"
...
def sync_questions_selection_to_master_tables(grande_area: str, subject: str, topic: str):
    grande_area = normalizar_texto(grande_area)
    subject = normalizar_texto(subject)
    topic = normalizar_texto(topic)

    if not grande_area or not subject or not topic:
        return

    try:
        ga_id = obter_ou_criar_grande_area(grande_area)
        tema_id = obter_ou_criar_tema(ga_id, subject)
        obter_ou_criar_subtopico(tema_id, topic)
    except Exception:
        pass
...
def add_study_session(
    ...
    try:
        ...
        sync_questions_selection_to_master_tables(grande_area, subject, topic)
...
def render_questions_review_panel(review_df: pd.DataFrame, user_id: int):
    st.markdown('<div class="b3-card">', unsafe_allow_html=True)
    st.markdown('<div class="b3-title">Revisão automática por rendimento</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="b3-sub">Regra aplicada: menor que 70% = 15 dias • 70 a 80% = 23 dias • maior que 80% = 30 dias.</div>',
        unsafe_allow_html=True
    )
...
def render_questions_subject_table(review_df: pd.DataFrame):
    st.markdown('<div class="b3-card">', unsafe_allow_html=True)
    st.markdown('<div class="b3-title">Mapa de revisão por assunto</div>', unsafe_allow_html=True)
    st.markdown('<div class="b3-sub">Consolidado automático dos assuntos a partir do desempenho nas questões.</div>', unsafe_allow_html=True)
...
def render_questions_manager():
    ...
    with c_top_1:
        ...
        st.markdown(
            '<div class="b3-sub">Insira uma sessão de questões. O sistema recalcula automaticamente a revisão do assunto.</div>',
            unsafe_allow_html=True
        )
    ...
    with left:
        st.markdown('<div class="b3-card">', unsafe_allow_html=True)
        st.markdown('<div class="b3-title">Assuntos com menor rendimento</div>', unsafe_allow_html=True)
        st.markdown('<div class="b3-sub">Prioridades de reforço a partir da acurácia acumulada.</div>', unsafe_allow_html=True)
...
def build_dashboard_metrics(user_id: int):
    ...
    best_topics = ranking_df.head(10).copy() if not ranking_df.empty else pd.DataFrame(columns=["topic_display", "accuracy", "questions_done"])
    worst_topics = ranking_df.sort_values(["accuracy", "questions_done"], ascending=[True, False]).head(10).copy() if not ranking_df.empty else pd.DataFrame(columns=["topic_display", "accuracy", "questions_done"])
...
def render_ranking_10_panel(metrics: dict):
    ...
    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown('<div class="exec-title">Top 10 melhores e piores subtópicos</div>', unsafe_allow_html=True)
    st.markdown('<div class="exec-sub">Leitura ampliada com os 10 maiores e 10 menores rendimentos.</div>', unsafe_allow_html=True)
...
def render_upcoming_reviews_panel(metrics: dict):
    ...
    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown('<div class="exec-title">Revisões perto de vencer</div>', unsafe_allow_html=True)
    st.markdown('<div class="exec-sub">Mostra revisões vencidas ou com vencimento nos próximos 7 dias.</div>', unsafe_allow_html=True)
        ...
        for _, row in review_df.iterrows():
            ...
            st.markdown(
                (
                    ...
                    f'<div class="b3-item-title">{html.escape(topic_display)}</div>'
                    f'<div class="b3-item-meta">{html.escape(" • ".join(meta))}</div>'
                    f'<div class="b3-item-meta" style="margin-top:6px;">{html.escape(review_status)}</div>'
                    '</div>'
                    f'<div class="b3-chip">{html.escape(normalize_text(row.get("priority_label", "")))}</div>'
                    '</div>'
                    '</div>'
                ),
                unsafe_allow_html=True
...
def render_schedule_manager():
    ...
    with left:
        ...
        with st.form("form_schedule_add", clear_on_submit=True):
            ...
            if st.session_state.get("is_admin", False):
                selected_targets = st.multiselect(
                    "Usuários que receberão este item",
                    target_labels,
                    default=[],
                    key="schedule_targets_admin"
                )
            else:
                selected_targets = [st.session_state.get("username", "")]
        ...
        if submitted:
            target_user_ids = resolve_selected_target_user_ids(selected_targets) if st.session_state.get("is_admin", False) else [int(st.session_state.user_id)]
            ok, msg = add_schedule_item_for_users(
                target_user_ids,
                week_no,
                area,
        ...
        if st.button("Importar CSV como cronograma executável", use_container_width=True, disabled=not csv_exists):
            if st.session_state.get("is_admin", False):
                target_user_ids = resolve_selected_target_user_ids(st.session_state.get("schedule_targets_admin", []))
            else:
                target_user_ids = [int(st.session_state.user_id)]
...
def inject_flashcard_fullscreen_css():
    st.markdown(
        ...
        .fc-fullscreen-wrap{
            ...
            border-radius: 0;
            background:
                radial-gradient(circle at top left, rgba(17,150,255,.10), transparent 26%),
                radial-gradient(circle at top right, rgba(110,97,255,.10), transparent 24%),
...
def render_flashcards_page():
    ...
    with left:
        ...
        with tab_manual_basic:
            ...
            with st.form("form_add_flashcard_basic", clear_on_submit=True):
                ...
                if st.session_state.get("is_admin", False):
                    selected_targets = st.multiselect(
                        "Usuários que receberão este flashcard",
                        target_labels,
                        default=[],
                        key="fc_basic_targets_admin"
                    )
                else:
                    selected_targets = [st.session_state.get("username", "")]
        ...
        with tab_manual_cloze:
            ...
            with st.form("form_add_flashcard_cloze", clear_on_submit=True):
                ...
                if st.session_state.get("is_admin", False):
                    selected_targets_cloze = st.multiselect(
                        "Usuários que receberão este cloze",
                        target_labels,
                        default=[],
                        key="fc_cloze_targets_admin"
                    )
                else:
                    selected_targets_cloze = [st.session_state.get("username", "")]

                submitted_cloze = st.form_submit_button("Adicionar flashcard cloze")

            if submitted_cloze:
                target_user_ids = resolve_selected_target_user_ids(selected_targets_cloze) if st.session_state.get("is_admin", False) else [int(st.session_state.user_id)]
        ...
        with tab_import:
            ...
            with st.form("form_import_flashcards_dual", clear_on_submit=False):
                ...
                if st.session_state.get("is_admin", False):
                    selected_import_targets = st.multiselect(
                        "Usuários que receberão os cards importados",
                        target_labels,
                        default=[],
                        key="fc_import_targets_admin"
                    )
                else:
                    selected_import_targets = [st.session_state.get("username", "")]

                import_submitted = st.form_submit_button("Importar arquivos")

            if import_submitted:
                target_user_ids = resolve_selected_target_user_ids(selected_import_targets) if st.session_state.get("is_admin", False) else [int(st.session_state.user_id)]
    ...
    with right:
        ...
        deck_delete_options = ["Selecione"] + decks

        if st.session_state.get("is_admin", False):
            delete_deck_targets = st.multiselect(
                "Usuários afetados",
                target_labels,
                default=[],
                key="fc_delete_deck_targets_admin"
            )
        else:
            delete_deck_targets = [st.session_state.get("username", "")]
        ...
        if st.button("Excluir deck inteiro", key="fc_delete_full_deck_btn", use_container_width=True):
            if deck_to_delete == "Selecione":
                ...
                if st.session_state.get("is_admin", False):
                    target_user_ids = resolve_selected_target_user_ids(delete_deck_targets)
...
def render_admin_page():
    ...
    st.markdown('<div class="b5-card">', unsafe_allow_html=True)
    st.markdown('<div class="b5-title">Visualizar estatísticas de aluno</div>', unsafe_allow_html=True)
    st.markdown('<div class="b5-sub">Selecione um aluno para abrir visão geral, relatório e diagnóstico completos.</div>', unsafe_allow_html=True)
...
def render_footer_premium():
    ...
    ">
        🩺 <b>Nexo Med</b> • Plataforma premium de acompanhamento • Mentoria do Jhon - Whatsapp (63)99937-2522     </div>
    """)
...
def build_situational_diagnosis(report: dict):
    ...
    phase_name = normalize_text(goal.get("phase_name", "Intermediária")) or "Intermediária"
...
def generate_pdf_report(report: dict, diagnosis: dict, username: str = ""):
    ...
    draw_pdf_highlight_box(
        ...
        [
            f"{goal.get('phase_name', 'Intermediária')}",
...
def render_reports_content(user_id: int, username_for_pdf: str = ""):
    ...
    with col1:
        ...
        for name, text in [
            ("Etapa atual", f"{goal.get('phase_name', 'Intermediária')}"),
...
def main():
    ...
    is_flashcard_fullscreen = (
        current_menu == "Flashcards" and
        st.session_state.get("flashcard_fullscreen", False)
    )
